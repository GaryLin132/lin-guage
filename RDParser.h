...
token peek_token(token *token_arr, size_t *index, int offset);
...
AST_node *primary_expr(token *token_arr, size_t *idx)
{
	...
	switch (peek_token(token_arr, idx, 0).type)
	{
	case NUMBER:
	{
		AST_node *node = create_treeNode('N', 2);
		node->val = peek_token(token_arr, idx, 0).val;
		eat_token(idx);
		return node;
	}
	case SEPERATOR:
	{
		...
		if (peek_token(token_arr, idx, 0).type == SEPERATOR || peek_token(token_arr, idx, 0).str[0] == ')')
		{
			eat_token(idx); // eat up ")"
			return node;
		}
	...
	case IDENTIFIER:
	{
		...
		strcpy(node->id, peek_token(token_arr, idx, 0).str);
		eat_token(idx);
		return node;
	}
	}
...
AST_node *multiplicative_expr(token *token_arr, size_t *idx)
{
	...
	while (peek_token(token_arr, idx, 0).type != NUMBER && (peek_token(token_arr, idx, 0).str[0] == '*' || peek_token(token_arr, idx, 0).str[0] == '/' || peek_token(token_arr, idx, 0).str[0] == '%'))
	{
		oper = create_treeNode(peek_token(token_arr, idx, 0).str[0], 2);
		eat_token(idx);
...
AST_node *additive_expr(token *token_arr, size_t *idx)
{
	...
	while (peek_token(token_arr, idx, 0).type != NUMBER && (peek_token(token_arr, idx, 0).str[0] == '+' || peek_token(token_arr, idx, 0).str[0] == '-'))
	{
		oper = create_treeNode(peek_token(token_arr, idx, 0).str[0], 2);
		eat_token(idx);
...
int expr_len(token *token_arr, size_t idx)
{
	...
	while (1)
	{
		...
		else if (token_arr[idx + len].type != IDENTIFIER && token_arr[idx + len].type != NUMBER && token_arr[idx + len].type != OPERATOR)
		{
			printf("invalid: forget semicolon\n");
			exit(2);
		}
